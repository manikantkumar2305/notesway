...
from utils import time_utils, id_utils
...
def _tokens(user: dict) -> auth_schema.TokenResponse:
    ...
    return auth_schema.TokenResponse(access_token=access, refresh_token=refresh)


@router.post("/register-college", response_model=auth_schema.AuthResponse, status_code=status.HTTP_201_CREATED)
async def register_college(payload: auth_schema.RegisterCollegeRequest, db=Depends(get_db)):
    ...
    token = _tokens(user)
    return auth_schema.AuthResponse(token=token, user=_serialize_user(user), college=_serialize_college(college))


@router.post("/register", response_model=auth_schema.AuthResponse, status_code=status.HTTP_201_CREATED)
async def register_user(payload: auth_schema.RegisterUserRequest, db=Depends(get_db)):
...
async def login(payload: auth_schema.LoginRequest, db=Depends(get_db)):
    ...
    if not user or not hashing.verify_password(payload.password, user["password"]):
        raise security.http_error(status.HTTP_401_UNAUTHORIZED, "Invalid credentials", code="invalid_credentials")
...
async def refresh_tokens(payload: auth_schema.RefreshRequest, db=Depends(get_db)):
    decoded = jwt.decode_token(payload.refresh_token)
    if decoded.get("type") != "refresh":
        raise security.http_error(status.HTTP_401_UNAUTHORIZED, "Invalid refresh token", code="unauthorized")

    user = await user_service.get_by_id(db, decoded.get("sub"))
    if not user or not user.get("isActive", True):
        raise security.http_error(status.HTTP_401_UNAUTHORIZED, "User not found", code="unauthorized")
    ...
    await db.get_collection("users").update_one(
        {"_id": user["_id"]},
        {"$set": {"tokenVersion": new_version, "updatedAt": time_utils.now_utc()}},
    )
...
async def logout(current_user=Depends(get_current_user), db=Depends(get_db)):
    await user_service.increment_token_version(db, str(current_user["_id"]))
